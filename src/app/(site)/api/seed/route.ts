...
export async function GET(request: Request) {
  ...
  log.push(`Created admin user: ${adminUser.email}`);

  // ─── Clear existing data ────────────────────────────────────────────────────
  const collections = ["users", "projects", "posts", "experience", "testimonials"] as const;
  for (const col of collections) {
    const existing = await payload.find({ collection: col, limit: 200 });
    for (const doc of existing.docs) {
      await payload.delete({ collection: col, id: doc.id });
    }
    log.push(`Cleared ${col}`);
  }
  ...
  for (const post of posts) {
    ...
    log.push(`Created post: ${post.title}`);
  }
  ...
  for (const exp of experiences) {
    ...
    log.push(`Created experience: ${exp.role} @ ${exp.company}`);
  }
